...
function AllSession() {
  ...
  useEffect(() => {
    (async () => {
      try {
        console.log(filter);
        let { data } = await API_CALL.session.get(filter);
        console.log(data);
  ...
  return (
    <PageContainer title="Ment Tech Leads" description="">
...
const SingleRow2 = ({ setOpen, reloadPage, index, row }) => {
  ...
  const DeleteEnquiry = async () => {
    try {
      ...
      const { data } = await API_CALL.session.delete({ id: row._id });
      if (data.success) {
        toast.success(data.message);
        reloadPage();
      }
  ...
  return (
    ...
    >
      ...
      <TableCell className="tablebox">{row?.ip}</TableCell>
      <TableCell className="tablebox">{row?.sessionId}</TableCell>
      <TableCell className="tablebox">{row?.url}</TableCell>
      <TableCell className="tablebox" align="left">
        {showDate(row?.createdAt)}
      </TableCell>
