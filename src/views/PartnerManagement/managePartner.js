...
import AddCategory from "./addPartner";
import { toast } from "react-toastify";
import DeleteCategory from "./DeleteConfirmation";
import EditCategory from "./EditPartner";
import { MyTheme } from "src/layouts/customTheme";
import NftCard, { CommunityCard, PartnerCard } from "src/components/CommonComponents/NftCard";
...
function ManagePartner() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setOpen(false);
        let { data } = await API_CALL.Partner.get({});
        console.log(data);
        if(data.success){
  ...
  return (
    <PageContainer title="Sky Dog Partner Management" description="">
      <DashboardCard title="Partner" action={ActionButton}>
...
return  (
  <Grid container spacing={2}>
    ...
    {rows?.map((item,key)=>{
  return <Grid item xs={12} sm={6}  md={4}><PartnerCard  setEditModal={setEditModal} reloadIt={reloadIt} key={item._id} item={item}/> </Grid>
})}
  </Grid>
